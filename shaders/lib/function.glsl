...
vec3 scatter_vector=vec3(1,step(0,pixel_world_theta_c)*sun_atten_g*pow(abs(pixel_world_theta_c),0.75),step(0,pixel_world_theta_c)*sun_atten_b*pow(abs(pixel_world_theta_c),1.5));
// 星空渲染
const float fixedYaw   = radians(-90.0);   // 滚动
const float fixedPitch = radians(60.0);   // 上下仰
vec3 sunAxis = rotateVectorAroundAxis(vec3(0,0,1),vec3(1,0,0),radians(-23.5));
...
float fog_sun_fun_max=10 ;//16.92 10.14
			...
			float fog_world_k=0.2+0.8*pow(pixel_world_theta_s,fog_world_fun);
